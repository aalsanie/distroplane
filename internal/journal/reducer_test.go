...
func TestReduceRequiredLifecycleEvents(t *testing.T) {
	...
	events := []Event{
		...
		event(3, EventLeaseAcquired, "op-a", "target-a", Payload{Lease: leasePayload("lease-a", "worker-a", 3, 30)}),
		...
		event(9, EventLeaseReleased, "op-a", "target-a", Payload{Lease: leasePayload("lease-a", "worker-a", 3, 30)}),
		event(10, EventOperationReady, "op-b", "target-a", Payload{}),
		event(11, EventLeaseAcquired, "op-b", "target-a", Payload{Lease: leasePayload("lease-b", "worker-a", 11, 12)}),
		event(12, EventLeaseExpired, "op-b", "target-a", Payload{Lease: leasePayload("lease-b", "worker-a", 11, 12)}),
		event(13, EventOperationCancelled, "op-b", "target-a", Payload{ErrorCode: "DEPENDENCY_TERMINAL"}),
		event(14, EventOperationReady, "op-c", "target-b", Payload{}),
		event(15, EventLeaseAcquired, "op-c", "target-b", Payload{Lease: leasePayload("lease-c1", "worker-a", 15, 40)}),
		attemptStarted(16, "op-c", "target-b", 1),
		event(17, EventProviderProcessStarted, "op-c", "target-b", Payload{Attempt: 1}),
		dispatched(18, "op-c", "target-b", 1),
		event(19, EventProviderResponseReceived, "op-c", "target-b", Payload{Attempt: 1}),
		event(20, EventOperationWaitingExternal, "op-c", "target-b", Payload{Attempt: 1, ProviderState: "review", Evidence: evidence(`{"ref":"c"}`)}),
		event(21, EventLeaseReleased, "op-c", "target-b", Payload{Lease: leasePayload("lease-c1", "worker-a", 15, 40)}),
		event(22, EventLeaseAcquired, "op-c", "target-b", Payload{Lease: leasePayload("lease-c2", "worker-a", 22, 50)}),
		reconcileStarted(23, "op-c", "target-b", 1),
		event(24, EventProviderProcessStarted, "op-c", "target-b", Payload{Attempt: 1}),
		event(25, EventProviderResponseReceived, "op-c", "target-b", Payload{Attempt: 1}),
		event(26, EventOperationPublished, "op-c", "target-b", Payload{Attempt: 1, ProviderState: "published", Evidence: evidence(`{"ref":"c"}`)}),
		event(27, EventLeaseReleased, "op-c", "target-b", Payload{Lease: leasePayload("lease-c2", "worker-a", 22, 50)}),
		event(28, EventRunCompleted, "", "", Payload{}),
	}
...
func TestReduceRejectsInvalidTransitions(t *testing.T) {
	...
	leaseA := Payload{Lease: leasePayload("lease-a", "worker-a", 2, 10)}
	leaseB := Payload{Lease: leasePayload("lease-b", "worker-a", 2, 10)}
	cases := map[string][]Event{
		...
		"lease before ready":               {runStarted(1), event(2, EventLeaseAcquired, "op-b", "target-a", leaseA)},
		"duplicate lease":                  {runStarted(1), event(2, EventLeaseAcquired, "op-a", "target-a", leaseA), event(3, EventLeaseAcquired, "op-a", "target-a", leaseA)},
		"lease expiry without acquire":     {runStarted(1), event(2, EventLeaseExpired, "op-a", "target-a", leaseA)},
		"premature lease expiry":           {runStarted(1), event(2, EventLeaseAcquired, "op-a", "target-a", leaseA), event(3, EventLeaseExpired, "op-a", "target-a", leaseA)},
		"mismatched lease renewal":         {runStarted(1), event(2, EventLeaseAcquired, "op-a", "target-a", leaseA), event(3, EventLeaseRenewed, "op-a", "target-a", leaseB)},
		"nonextending lease renewal":       {runStarted(1), event(2, EventLeaseAcquired, "op-a", "target-a", leaseA), event(3, EventLeaseRenewed, "op-a", "target-a", Payload{Lease: leasePayload("lease-a", "worker-a", 2, 9)})},
		"mismatched lease release":         {runStarted(1), event(2, EventLeaseAcquired, "op-a", "target-a", leaseA), event(3, EventLeaseReleased, "op-a", "target-a", leaseB)},
