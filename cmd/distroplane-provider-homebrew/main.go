...
var version = homebrewprovider.DefaultVersion
...
func execute(ctx context.Context, streams process) int {
	...
	if providerVersion == "" {
		providerVersion = homebrewprovider.DefaultVersion
	}
	return run(ctx, streams.input, streams.output, streams.diagnostics, commandProvider(providerVersion))
}

func commandProvider(providerVersion string) homebrewprovider.Provider {
	return homebrewprovider.Provider{Version: providerVersion}
}

func run(ctx context.Context, input io.Reader, output, diagnostics io.Writer, provider homebrewprovider.Provider) int {
