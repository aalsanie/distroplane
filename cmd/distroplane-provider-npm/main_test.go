...
func TestRunDescribe(t *testing.T) {
	...
	if code := run(context.Background(), &input, &output, &diagnostics, npmprovider.Provider{Version: "1.2.3"}); code != 0 {
	...
	if description.Provider.Name != npmprovider.Name || description.Provider.Version != "1.2.3" {
...
func TestRunInvalidInput(t *testing.T) {
	var output, diagnostics bytes.Buffer
	if code := run(context.Background(), bytes.NewBufferString("{"), &output, &diagnostics, npmprovider.Provider{}); code != 1 {
...
func TestExecuteUsesDefaultVersion(t *testing.T) {
	...
	if description.Provider.Version != npmprovider.DefaultVersion {
